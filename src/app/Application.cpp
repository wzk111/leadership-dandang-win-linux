...
#include <QSysInfo>
namespace ws {
Application::Application(AppController& controller, ISecretStore& secrets, QSettings& settings)
    : controller_(controller), secrets_(secrets), settingsStore_(settings), settings_(settings, secrets) {
    ...
    diagnostics_.setWindowTitle("WorkSidekick — Diagnostics"); diagnostics_.resize(600, 480);
    ...
    connect(selection, &QPushButton::clicked, this, [this] { testStatus_->setText("Selection capture is not implemented in M0. Use manual clipboard mode."); });
